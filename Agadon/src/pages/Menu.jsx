...
const Menu = () => {
  ...
  const goldenTime = useMemo(() => {
    ...
    return new Date(lastTrainTime.getTime() - walkInfo.minutes * 60 * 1000);
  }, [walkInfo, lastTrainTime]);
      ...
      <div className="flex justify-center items-center gap-3 my-4">
        {[30, 15, 5, 0].map((min) => (
          <button
            ...
            className=" text-white text-xs px-4 py-2 rounded-full active:bg-green-400 transition font-bold"
          >
            {min === 0 ? '정각' : `${min}분전`}
          </button>
        ))}
